...
class SimpleClassifierWithExitLossGradual(nn.Module):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if is_naive:
                ...
                # Exit Loss Gradual Curriculum
                progress = epoch / epochs
                curriculum_weights = [(1 - progress) * (idx + 1) / len(aux_losses) + progress
                                      for idx in range(len(aux_losses))]
                total_weight = sum(curriculum_weights)
                curriculum_weights = [w / total_weight for w in curriculum_weights]

                if batch_idx == 0 and epoch % 1 == 0:
                    print(f"Curriculum weights at epoch {epoch}: {curriculum_weights}")

                weighted_aux_losses = [aux_loss_weight * w * l
                                       for w, l in zip(curriculum_weights, aux_losses)]
        ...
        correct = 0
        if not is_naive:
            layer_exits_count = [0] * (len(model.layers) + 1)
...
if __name__ == "__main__":
    ...
    # Save trained models
    torch.save(model_with_skip.state_dict(), 'model_with_layerskip_gradual_exit_loss.pt')
